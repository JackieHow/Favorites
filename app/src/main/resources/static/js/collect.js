...
function listStandardCollect(collects, listId, user) {
    ...
    for (var i = 0; i < collects.length; i++) {
        ...
        var item =
            "<li>" +
            "<a style=\"background-image:url(" + (collects[i].profilePicture == '' ? 'img/favicon1.png' : '/' + collects[i].profilePicture) + ")\" class=\"hidden-xs timeline-badge sharing-user-avatar\" href=\"javascript:void(0);\" onclick=\"locationUrl(\'/user/" + collects[i].userId + "/0\',\'\');\" ></a>" +
        ...
        item = item +
            ...
            "               <div style=\"background-image:url(" + (collects[i].logoUrl == '' ? 'img/favicon1.png' : collects[i].logoUrl) + ")\" class=\"media-object resource-card-image\"></div>" +
            "            </a>" +
...
function listSimpleCollect(collects, user) {
    ...
    for (var i = 0; i < collects.length; i++) {
        var item =
            ...
            "     <img height=\"25px\" width=\"35px\" src=\"" + (collects[i].logoUrl == '' ? 'img/favicon1.png' : collects[i].logoUrl) + "\" alt=\"\"></td>" +

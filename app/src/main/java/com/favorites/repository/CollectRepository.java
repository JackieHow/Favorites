...
public interface CollectRepository extends JpaRepository<Collect, Long> {
    ...
    Page<CollectView> findExploreViewByCategory(Pageable pageable);
    ...
    @Transactional
    @Modifying
    @Query("select url from Collect where favoritesId = ?1 and url in ?2 and userId = ?3 and isDelete = ?4")
    List<String> findByFavoritesIdAndUrlAndUserIdAndIsDelete1(Long favoritesId, List<String> url, Long userId, IsDelete isDelete);
    ...
    @Query("select c.id as id,c.title as title, c.type as type,c.url as url,c.logoUrl as logoUrl,c.userId as userId, "
            + "c.remark as remark,c.description as description,c.lastModifyTime as lastModifyTime,c.createTime as createTime, "
            + "u.userName as userName,u.profilePicture as profilePicture,f.id as favoritesId,f.name as favoriteName "
            + "from Collect c,User u,Favorites f WHERE c.userId=u.id and c.favoritesId=f.id and c.userId in (select f1.followId FROM  Follow f1 WHERE f1.userId in ?1 and f1.status='FOLLOW'  )  and c.type = 'PUBLIC' and c.isDelete='NO' ")
    Page<CollectView> findDiscoverView(Long userId, Pageable pageable);
